...
export default function page() {
    ...
    return (
        <div className={`flex ${isAdmin && "justify-center"}  h-screen`}>
            <div
                className={`${isAdmin ? "w-full pr-2 xs:px-6 sm:px-12 md:px-16" : dimensions.width >= 720
                    ...
                    } flex flex-col items-center pt-8 pb-20 overflow-y-auto h-screen `}
            >
                {isAdmin && <WritePost announcement={true} adminAnnouncement={true} />}
                <ShowPost announcement={true} />
            </div>
            {!isSmallScreen && !isAdmin && (
                <div
                    className={`${dimensions.width < 950 ? 'w-4/12' : 'w-[28%]'
                        } h-screen mr-4 overflow-y-auto overflow-x-hidden fixed pl-4 top-[64px] right-0`}
